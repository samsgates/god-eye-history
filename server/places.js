...
function stripDate(q){return q.replace(/\b\d{1,2}\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4}\b/ig,'').replace(/\b(on|in|during)\b$/i,'').trim()}

export async function searchPlaces(q){
  const date=parseDateIntent(q);const placeQ=stripDate(q)||q;
  return cached(`place:${placeQ}`,3600,async()=>{
    ...
    const places=list.map(x=>({id:`osm:${x.osm_type}:${x.osm_id}`,name:x.display_name?.split(',')[0]||x.name,display_name:x.display_name,lat:Number(x.lat),lng:Number(x.lon),country:x.address?.country,region:x.address?.state||x.address?.region,type:x.type||x.addresstype,bbox:x.boundingbox}));
    return {places,intent:{date,query:q,placeQuery:placeQ}};
  });
}
